...
import ViewRejectedCandi from "../../pages/Owner/viewRejectedCandi";
...
describe("Testing candidate selection page", () => {
  ...
  afterEach(() => {
    candidates.candiInfo[1].candiStatus = "Rejected"
    server.resetHandlers();
 
});
  afterAll(() => server.close());

  it("WHEN component is mounted THEN render the list of rejected candidate", async () => {
    loadUser()
    getCandidateList();
    const {debug,queryByText} = await act(()=> render(<ViewRejectedCandi />, {
      route: "/Owner/viewRejectedCandi/",
    })
    );
    expect(queryByText("List of Rejected Candidates")).toBeInTheDocument()
    expect(queryByText("CAND0004")).toBeInTheDocument()
  });
  ...
  it("WHEN click on reject button THEN reject Modal should render and after filling the render candidate should remove from page", async () => {
    ...
    const {debug,queryByText,getAllByTestId,getByTestId,findByText,queryByTestId} = await act(()=> render(<ViewRejectedCandi />, {
      route: "/Owner/viewRejectedCandi/",
    })
    );
    await findByText("CAND0004")

    const allseeDetailsBtn = getAllByTestId("seeDetailsBtn")

    expect(allseeDetailsBtn.length).toBeGreaterThanOrEqual(1);

    await waitFor(()=>userEvent.click(allseeDetailsBtn[0]))
    expect(getByTestId("reject-Message-Model")).toBeInTheDocument()

    fireEvent.click(getByTestId("closeRejectModal"))
    expect(queryByTestId("reject-Message-Model")).not.toBeInTheDocument()
  });
