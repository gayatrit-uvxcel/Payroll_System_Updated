...
import ViewSelectedCandi from "../../pages/Owner/viewSelectedCandi";
...
describe("Testing candidate selection page", () => {
  ...
  it("WHEN component is mounted THEN render the list of selected candidate", async () => {
    loadUser()
    getCandidateList();
    const {debug,queryByText} = await act(()=> render(<ViewSelectedCandi />, {
      route: "/Owner/viewSelectedCandi/",
    })
    );
    expect(queryByText("List of Selected Candidates")).toBeInTheDocument()
    expect(queryByText("CAND0013")).toBeInTheDocument()
  });//

//   onboading button
  it("WHEN click on onboarding button  THEN render candidate should remove from page", async () => {
    loadUser()
    getCandidateList({candidateData:candidates});
    const {debug,queryByText,getAllByTestId,findByText} = await act(()=> render(<ViewSelectedCandi />, {
      route: "/Owner/viewSelectedCandi/",
    })
    );
    await findByText("CAND0013")

    const allOnboardButton = getAllByTestId("onboardBtn")

    expect(allOnboardButton.length).toBeGreaterThanOrEqual(1);

    server.use(rest.put("/api/v2/edit-candi/CAND0013",(req,res,ctx)=>{
        candidates.candiInfo[0].candiStatus = "Onboard"
  ...
  it("WHEN click on reject button THEN reject Modal should render and after filling the render candidate should remove from page", async () => {
    loadUser()
    getCandidateList({candidateData:candidates});
    const {debug,queryByText,getAllByTestId,getByTestId,findByText} = await act(()=> render(<ViewSelectedCandi />, {
      route: "/Owner/viewSelectedCandi/",
    })
    );
    await findByText("CAND0013")
