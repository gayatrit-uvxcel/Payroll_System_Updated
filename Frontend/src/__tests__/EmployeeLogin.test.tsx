...
import Modal from "../pages/Profile1"
...
describe("empConfirm.test.tsx", () => {
  ...
  it("checking chekbox is working or not", async () => {
    //Calling custome server
    ...
    // redering component here
    const { debug, findByText, queryAllByTestId,getByLabelText,getAllByTestId,container } = render(<Profile />, {
      route: "/app/profile1",
    });
    //checking that text is present or not
    await findByText("Prapti Anil Gomekar");
    const checkbox = getAllByTestId("check1")[0] as HTMLInputElement;
  ...
  expect(checkbox).toBeChecked();
    ...
    //fireEvent.click(familyModal);
   // expect(familyModal.value).toBeTruthy();
    debug()
  });
